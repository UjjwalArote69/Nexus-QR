...
const useQRStore = create((set, get) => ({
  ...
  setFgColor: (fgColor) => {
    if (/^#[0-9a-fA-F]{0,6}$/.test(fgColor)) set({ fgColor });
  },
  setBgColor: (bgColor) => {
    if (/^#[0-9a-fA-F]{0,6}$/.test(bgColor)) set({ bgColor });
  },
  ...
  setLogo: (file) => {
    ...
    reader.onload = (e) => set({ logoFile: file, logoDataUrl: e.target.result });
  ...
  createQRCode: async (qrData) => {
    ...
    try {
      ...
      if (result.success) {
        set({ isLoading: false, pendingQRData: null });
        const token = localStorage.getItem('token');
        if (!token) {
          // Small delay so the user sees the success state first
          setTimeout(() => set({ showAuthPrompt: true }), 1500);
        }
  ...
  createQRWithFileAction: async (file, title, qrType) => {
    ...
    try {
      ...
      const result = await createQRWithFile(formData);

      if (result.success) {
        set({ isLoading: false, pendingFileData: null });
        const token = localStorage.getItem('token');
        if (!token) {
          // Small delay so the user sees the success state first
          setTimeout(() => set({ showAuthPrompt: true }), 1500);
  ...
  retryPendingCreation: async () => {
    const { pendingQRData, pendingFileData } = get();

    if (!pendingQRData && !pendingFileData) {
      return { success: false, message: 'No pending QR data.' };
    }

    set({ showAuthPrompt: false, isLoading: true, error: null });
    ...
    try {
      ...
      if (pendingFileData) {
        ...
        formData.append('title', pendingFileData.title);
        formData.append('qrType', pendingFileData.qrType);
        result = await createQRWithFile(formData);
      } else {
        // Retry regular QR creation
        result = await generateQRCode(pendingQRData);
      }

      if (result.success) {
        set({ isLoading: false, pendingQRData: null, pendingFileData: null });
        return result;
      } else {
        set({
          error: result.message || 'Failed to generate QR code.',
      ...
      const errMsg = error.message || 'Failed to save QR code after login.';
      set({ error: errMsg, isLoading: false });
      return { success: false, message: errMsg };
    }
  },

  dismissAuthPrompt: () => set({ showAuthPrompt: false, pendingQRData: null, pendingFileData: null }),
}));
